...
try {
	...
	while ($obj = $stmt-> fetch (PDO::FETCH_NUM)) {
      ...
      <div class='col s12 m12'>
        <div class='card-panel deep-orange accent-1'>
          <span class='white-text'>Prezado, ".utf8_decode($obj[1]).". Enviamos para seu email cadastrado um link para criar uma nova senha. Dentro de instantes, acesse este email para recuperar!<span class='red-text'> Caso n&atildeo receba a senha, verifique a caixa de spam/lixo eletr&ocircnico ou entre em contato conosco pelo email: </span>
		  <a title='contato@viciadosemsushi.com.br' href='mailto:contato@viciadosemsushi.com.br'>contato@viciadosemsushi.com.br</a>
          </span>
        </div>
      </div>
    </div>";
	
	$linkMensagem = "http://viciadosemsushi.com.br/recoverymypass.php?token=".$token."&code=".$nomeCodificado."";
	$subjectMensagem = "Recupera&ccedi&atilde de senha - VICIADOS EM SUSHI";
	$msgHTMLMAIL ='
...
<head>
  ...
  <title>VICIADOS EM SUSHI</title>
</head>
<body>
<div style="width: 640px; font-family: Arial, Helvetica, sans-serif; font-size: 11px;">
  <h1>MENSAGEM DE RECUPERAÇÃO DE SENHA.</h1>
  <div align="center">
    
  </div>
  <h4>Prezado cliente, <strong>'.$obj[1].'. </strong>Para mudar a sua senha, clique no link abaixo e realize a mudança em nosso site.</h4>
  <p><a href='.$linkMensagem.'>'.$linkMensagem.'</a>
</div>
</body>
</html>
...
//sleep(3); //aguarda 3 segundos
	...
	$clienteEmailLow = strtolower($clienteEmail);
...
try
    ...
    $pdo = new PDO ( "mysql:host={$DB_Host};dbname={$DB_Name}", $DB_User, $DB_Pass);
		...
		$stmtEmail = $pdo->prepare("SELECT * FROM clientes WHERE clientes_Email = :email");
 
        $stmtEmail -> bindValue(':email', $clienteEmail);
		
		$stmtEmail -> execute();
		
		while ($obj = $stmtEmail-> fetch (PDO::FETCH_NUM)) {
 ...
 try {
	...
	//$openMarket = "Loja Fechada!";
      ...
      <div class='col s12 m12'>
        <div class='card-panel red'>
          <span class='white-text'>Prezado cliente, o email digitado j&aacute consta na nossa base de dados. Caso voc&ecirc tenha cadastro conosco mas perdeu a senha, ou n&atildeo tinha cadastrado a senha ainda, acesse nosso menu -> Login -> Esqueci a senha.
          </span>
        </div>
      </div>
    </div> ";
	...
	$stmt -> bindValue(':Email',$clienteEmailLow);
...
try
	...
	if ($result)
	{
		...
		$subjectMensagem = "Descritivo do pedido - VICIADOS EM SUSHI";
	$msgHTMLMAIL ='
...
<body>
<div style="width: 640px; font-family: Arial, Helvetica, sans-serif; font-size: 11px;">
  <h1>NOTA DE COMPRA - VICIADOS EM SUSHI.</h1>
  <div align="center">
    
  </div>
  <h4>Prezado cliente, <strong>'.$_SESSION['NAMECLI'].'. </strong> Obrigado por comprar conosco! Dentro de instantes, estaremos enviando seu pedido. Para acompanhar o status do pedido e o tempo de entrega, entre com seu login no site e clique na cesta de compras.</h4>
  <h5>Pedido: '.$descritivosConsolidados.'</h5>
  <h5>Valor: R$ '.number_format($total, 2, ',', '.').'</h5>
  <p><a href="http://viciadosemsushi.com.br/">ACESSE A LOJA E ACOMPANHE O SEU PEDIDO</a>
</div>
</body>
</html>
...
try
	...
	//$openMarket = "Loja Fechada!";
      ...
      <div class='col s12 m12'>
        <div class='card-panel red'>
          <span class='white-text'>Ja existe esse email cadastrado, o email digitado j&aacute consta na nossa base de dados. Caso o cliente nao tenha conhecimento do cadastro, solicite outro email para cadastro.
          </span>
        </div>
      </div>
    </div> ";
	...
	while ($objMail = $stmtEmail-> fetch (PDO::FETCH_NUM)) {
		
		$EmailREtornadoIDCLIENTE = $objMail[0];
		
	}
		
		
		$token = geraSenha(20, true, true);
		$nomeCodificado = base64_encode($clienteNome);
		$linkMensagem = "http://viciadosemsushi.com.br/recoverymypass.php?token=".$token."&code=".$nomeCodificado."";
	    $subjectMensagem = "Criação de senha - VICIADOS EM SUSHI - FINALIZAR CADASTRO";
	    $msgHTMLMAIL ='
...
<body>
<div style="width: 640px; font-family: Arial, Helvetica, sans-serif; font-size: 11px;">
  <h1>MENSAGEM DE CRIAÇÃO DE SENHA.</h1>
  <div align="center">
    
  </div>
  <h4>Prezado cliente, <strong>'.$clienteNome.'. </strong>Você foi cadastrado no site do VICIADOS EM SUSHI e para maior comodidade, finalize o cadastro escolhendo uma senha para acesso. Após isto, você poderá realizar pedidos no site, acompanhar as entregas dos pedidos. Tudo rápido e facil no seu smartphone ou computador</h4>
  <h4>Para realizar a mudança, clique no link abaixo e cadastre uma senha:</h4>
  <p><a href='.$linkMensagem.'>'.$linkMensagem.'</a>
</div>
</body>
</html>
	...
	';
		...
		$stmt2 -> bindValue(':Id_Cliente',$EmailREtornadoIDCLIENTE);
		$stmt2 -> bindValue(':Nome_Cliente',$clienteNome);
		$stmt2 -> bindValue(':Cod_Aut',$token);
		$stmt2 -> bindValue(':Ativo',$ativo);
		// Executa query
		$stmt2->execute();
	...
	echo"	 <div class='row'>
      <div class='col s12 m12'>
        <div class='card-panel teal'>
          <span class='white-text'>Cliente, ".$clienteNome.". Cadastrado com sucesso! Ele recebera um email para cadastrar a senha
          </span>
        </div>
      </div>
    </div> ";
