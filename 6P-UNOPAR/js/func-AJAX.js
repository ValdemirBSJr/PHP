...
function retornaCliente() {
	...
	var id      = document.getElementById("clienteConsulta").value; //Aqui eu pegava o critpério da busca que era o nome do filme, mas não preciso disso
	//var id = "tudo"; //atribuo um valor qualquer. Ele será descartado, se precisasse realizar a busca atribuia
	var result  = document.getElementById("result");
	var XMLHttp = generateXMLHttp();
	XMLHttp.open("get", "retornaCliente.php?id=" + id, true);
	XMLHttp.onreadystatechange = function(){
		if (XMLHttp.readyState == 4)
			if (XMLHttp.status == 200){
				result.innerHTML = XMLHttp.responseText;
